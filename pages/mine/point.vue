<template>
	<view class="user-point">
		...
		<view class="btn-row">
			<button type="default" @click="syncPointValue">同步积分数据 {{ stepInfoList.step || '' }}</button>
		</view>
	</view>
</template>
			...
			async getStepInfoList (weRunEncryptedData) {
				...
				uniCloud.callFunction({
					...
					success: (res) => {
						...
						this.stepInfoList = res.result.stepInfoList
						this.allPointRecords = res.result.allPointRecords.reverse().map(record => {
							record.time = getDate(record.date, 'year', '/')
							this.totalPoint += record.changePoints
							return record
						})
					},
				})
