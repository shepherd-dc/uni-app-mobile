<template>
	<view class="list-container">
		<view class="list" v-for="(l, i) in list" :key="l._id">
			<view class="list-item" v-if="l.vaccine.length">
				...
				<view class="child-item" v-for="v in l.vaccine" :key="v._id">
							...
							<view class="title">
								<view class="title-main">
									<text
										v-if="tag"
										class="type"
										:class="{'self-pay': v.type === 1}">{{ v.type === 1 ? '自费' : '免费' }}</text>
						...
						<view class="list-extra">
							<template v-if="extra === 'checkbox'">
								<checkbox-group @change="checkboxChange">
									<label>
										<text class="date" v-if="done">{{ nowDate }}</text>
										<checkbox :value="v._id" :checked="v.done"/>
									</label>
								</checkbox-group>
							</template>
							<template v-if="extra === 'button'">
								<view class="button" type="primary">未添加</view>
							</template>
						</view>
					</view>
					<view class="list-footer" v-if="v.tips">
