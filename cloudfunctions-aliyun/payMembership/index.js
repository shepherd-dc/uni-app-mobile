...
exports.main = async(event, context) => {
  const { openid, membershipPlanId } = event
  var result = false
  var errMsg = 'ok'
	var errCode = 0
  //读取用户信息
  var user = (await db.collection('user')
    ...
    .get()).data[0]
  //读取用户等级折扣
  var levels = (await db.collection('user-level').get()).data
  ...
  if (user.point < paymentFee) {
    ...
    await db.collection('user_point')
      .add({
				...
				operation: "购买会员",
				timestamp: new Date(Date.now() + (8 * 60 * 60 * 1000)).getTime(),
				orderId: ''
      })

    var memberExpDate = user.memberExpDate || 0 //用户原会员有效期
