...
<uni-shadow-root class="vant-dist-uploader-index"><view class="van-uploader">
  <view class="van-uploader__wrapper">
    ...
    <view v-for="(item,index) in (lists)" :key="item.index" v-if="previewImage" class="van-uploader__preview" :data-index="index" @click="onClickPreview">
      ...
      <video v-else-if="item.isVideo" :src="item.url || item.path" :autoplay="item.autoplay" class="van-uploader__preview-image" :style="'width: '+(utils.addUnit(previewSize))+'; height: '+(utils.addUnit(previewSize))+';'" :data-index="index" @click="onPreviewVideo">
      </video>      
